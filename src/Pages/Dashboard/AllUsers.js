...
const AllUsers = () => {
    ...
    const { data: users = [], refetch } = useQuery({
        queryKey: ['users'],
        queryFn: async () => {
            const res = await fetch('https://book-mart-server-1.vercel.app/users')
            const data = await res.json()
            return data
        }
    })
    const handleMakeAdmin = id => {
        fetch(`https://book-mart-server-1.vercel.app/users/admin/${id}`, {
            method: 'PUT',
            headers: {
...
/////////////
    const handleDeletDoctor = user => {
        ...
        fetch(`https://book-mart-server-1.vercel.app/users/${user}`, {
            method: 'DELETE',
            headers: {
                authorization: `bearer ${localStorage.getItem('accessToken')}`
            }
